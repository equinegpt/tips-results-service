...
def main() -> int:
    ...
    cur = conn.cursor()
    cur.execute("""
        CREATE TABLE IF NOT EXISTS lifeboat_connection_a2e (
            ...
            PRIMARY KEY (entity_type, entity_id, as_of))""")
    cur.execute("TRUNCATE lifeboat_connection_a2e")
    conn.commit()

    print("[a2e] pulling jockey runs…")
    cur.execute("""
        SELECT rr.jockey_id, r.race_date,
        ...
        WHERE r.race_date >= '2015-01-01'
          ...
          AND rr.odds_closing > 1 AND rr.jockey_id IS NOT NULL
          AND rr.position IS NOT NULL""")
    jock = snapshots(cur.fetchall(), "jockey")

    print("[a2e] pulling trainer runs (current-trainer attribution — declared v1 proxy)…")
    cur.execute("""
        SELECT h.trainer_id, r.race_date,
               CASE WHEN rr.position = 1 THEN 1 ELSE 0 END,
               1.0 / rr.odds_closing
        ...
        WHERE r.race_date >= '2015-01-01'
          AND COALESCE(rr.is_trial, false) = false
          AND rr.odds_closing > 1 AND h.trainer_id IS NOT NULL
          AND rr.position IS NOT NULL""")
    trn = snapshots(cur.fetchall(), "trainer")

    rows = jock + trn
    print(f"[a2e] inserting {len(rows)} snapshot rows…")
    psycopg2.extras.execute_values(cur, """
        INSERT INTO lifeboat_connection_a2e
            (entity_type, entity_id, as_of, career_runs, career_wins,
             career_exp, career_a2e, runs_365, wins_365, exp_365, a2e_365, sr_365)
        VALUES %s""", rows, page_size=2000)
    conn.commit()
    cur.execute("SELECT entity_type, count(DISTINCT entity_id), count(*) FROM lifeboat_connection_a2e GROUP BY 1")
